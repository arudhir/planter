...
def get_db_schema_version(db_path: Union[str, Path]) -> int:
    ...
    try:
        ...
        if "sequences" in tables_columns and "is_representative" in tables_columns["sequences"]:
            return 2
        else:
            return 1
...
def upgrade_schema(db_path: Union[str, Path], target_version: Optional[int] = None) -> int:
    """
    ...
    if current_version >= target_version:
        logger.info(f"Database already at schema version {current_version}, no upgrade needed")
...
def ensure_compatibility(db_path: Union[str, Path], required_version: int = None) -> Tuple[int, bool]:
    """
