...
-- The SEQHASH_IDS_PLACEHOLDER will be replaced with the actual list of IDs
...
WITH seq_info AS (
    ...
    WHERE s.seqhash_id IN (SEQHASH_IDS_PLACEHOLDER)
),
cluster_info AS (
    SELECT 
        cm.seqhash_id,
    ...
    WHERE cm.seqhash_id IN (SEQHASH_IDS_PLACEHOLDER)
),
go_terms_info AS (
    SELECT 
        g.seqhash_id,
    ...
    FROM go_terms g
    WHERE g.seqhash_id IN (SEQHASH_IDS_PLACEHOLDER)
...
ec_numbers_info AS (
    ...
    FROM ec_numbers e
    WHERE e.seqhash_id IN (SEQHASH_IDS_PLACEHOLDER)
...
expression_info AS (
    ...
    WHERE gpm.protein_seqhash_id IN (SEQHASH_IDS_PLACEHOLDER)
