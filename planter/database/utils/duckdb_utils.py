...
from planter.database.schema.schema_version import (
...
def merge_duckdbs(
    ...
    if upgrade_schema:
        schema_version, was_upgraded = ensure_compatibility(
            master_db_path, 
            required_version=target_schema_version
        )
        if was_upgraded:
    ...
    tables_by_version = {
        1: [
            "sra_metadata", "sequences", "annotations", "go_terms", 
            "ec_numbers", "kegg_info", "clusters", "cluster_members"
        ],
        2: [
            "sra_metadata", "sequences", "annotations", "go_terms", 
            "ec_numbers", "kegg_info", "clusters", "cluster_members",
            "expression", "gene_protein_map"
        ],
    }
    
    # Get the list of tables for the current schema version
    tables = tables_by_version.get(schema_version, tables_by_version[max(tables_by_version.keys())])
    ...
    with duckdb.connect(master_db_path) as master_conn:
        ...
        for i, source_db in enumerate(duckdb_paths):
            ...
            for table in tables:
                ...
                table_exists_in_source = master_conn.execute(f"""
                ...
                if table_exists_in_source:
                    ...
                    if common_cols:
                        ...
                        if (table == "sequences" and 
                            "is_representative" in target_col_names and 
                            "is_representative" not in source_col_names):
                        ...
                        logger.warning(f"No common columns found for table {table}, skipping import")
                    
                    # Log the number of inserted rows
                    row_count = master_conn.execute(f"""
...
def update_duckdb_with_cluster_info(db_path: Union[str, Path], tsv_path: Union[str, Path], upgrade_schema: bool = True) -> None:
    """
    ...
    schema_version, was_upgraded = ensure_compatibility(db_path) if upgrade_schema else (get_db_schema_version(db_path), False)
    if was_upgraded:
    ...
    try:
        logger.info(f"Updating database '{db_path}' with MMSeqs2 clustering information from '{tsv_path}'")
        ...
        if missing_seqs:
            ...
            logger.warning(f"{missing_count} sequence IDs from cluster file not found in the database")
            if missing_count > 10:
                logger.warning(f"First 10 missing IDs: {[m[0] for m in missing_seqs[:10]]}")
            else:
        ...
        logger.info(f"Successfully updated {updated_count} sequences with new cluster assignments")
        logger.info(f"Created {cluster_stats[0]} clusters with average size {cluster_stats[1]:.1f} (max: {cluster_stats[2]})")
    ...
    logger.info(f"Database '{db_path}' successfully updated with MMSeqs2 clustering information.")