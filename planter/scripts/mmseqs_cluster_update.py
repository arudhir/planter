...
class MMseqsClusterUpdater:
    ...
    def update_clusters(
        self, old_seqs: str, new_seqs: str
    ...
    def _run_initial_clustering(self, old_seqs: str) -> None:
        ...
        self.run_command(
            [
                ...
                self.tmp_dir,
                "--cluster-mode", "2",  # Use longest sequence as representative
            ]
        )
        self.run_command(
            [
                ...
                self.paths["sequenceDB"],
                self.paths["clusterDB"],
                os.path.join(self.output_dir, "clusterDB.tsv"),
            ]
        )
        self.run_command(
            [
        ...
        self.run_command(
            [
                ...
                self.paths["sequenceDB"],
                self.paths["alignDB"],
                os.path.join(self.output_dir, "align.m8"),
            ]
        )
    ...
    def _add_new_sequences(self, new_seqs: str) -> None:
        ...
        self.run_command(
            ["mmseqs", "createdb", new_seqs, self.paths["addedSequenceDB"]]
        )
        self.run_command(
            [
                "mmseqs",
        ...
        self.run_command(
            [
                "mmseqs",
                "concatdbs",
                f"{self.paths['sequenceDB']}_h",
                f"{self.paths['addedSequenceDB']}_h",
                f"{self.paths['allSequenceDB']}_h",
            ]
        )
    ...
    def _count_clusters(self) -> Tuple[int, int]:
        ...
        self.run_command(
            [
                ...
                self.paths["newSequenceDB"],
                self.paths["newClusterDB"],
                os.path.join(self.output_dir, "newClusterDB.tsv"),
            ]
        )
    ...
    def _compare_representatives(self) -> Tuple[int, int]:
        ...
        try:
            ...
            logging.error(
                f"Error comparing representative sequences: {e.output.decode()}"
            )
            return 0, 0
    ...
    def _generate_alignments(self) -> None:
        ...
        self.run_command(
            [
                ...
                self.paths["newSequenceDB"],
                self.paths["newAlignDB"],
                os.path.join(self.output_dir, "newAlign.m8"),
            ]
        )
...
class BatchMMseqsUpdater:
    ...
    def __init__(
        self, files: List[str], base_output_dir: str, initial_reps: Optional[str] = None
    ):
        self.files = files
    ...
    def process_files(self) -> None:
        ...
        if not old_rep_seqs and files:
            ...
            logging.info(
                f"No initial representative sequences provided. Using {first_srr} ({files[0]}) as initial input."
            )
            files = files[1:]
    ...
    def _log_iteration_stats(
        self, iteration: int, srr_id: str, stats: Tuple[int, int, int, int]
    ) -> None:
