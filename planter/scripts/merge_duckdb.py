...
def merge_databases(add_db: str, output_db: str):
    ...
    sample_ids = con.execute(
        "SELECT DISTINCT sample_id FROM new_sample.sra_metadata"
    ).fetchall()
    for sid_tuple in sample_ids:
        ...
        if existing_count > 0:
            print(
                f"Sample '{sample_id}' already exists in the master DB. Merge aborted."
            )
...
def main():
    ...
    parser.add_argument(
        "--current", required=True, help="Path to the current (master) DuckDB file."
    )
    parser.add_argument(
        "--add",
        required=True,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path for the output merged DuckDB file. "
    ...
    if output_path is None:
        tmp = tempfile.NamedTemporaryFile(
            prefix="merged_", suffix=".duckdb", delete=False
        )
