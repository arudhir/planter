...
def parse_eggnog(eggnog_file):
    ...
    cog_category_count = (
        df.groupby("COG_category")["COG_category"].agg("count").to_dict()
    )
...
def parse_fastp(fastp_file):
    ...
    stats = {}
    stats["Total Reads (Before)"] = fastp_output["summary"]["before_filtering"][
        "total_reads"
    ]
    stats["Total Reads (After)"] = fastp_output["summary"]["after_filtering"][
        "total_reads"
    ]
...
def calculate_exn50(lengths, expressions):
    ...
    sorted_indices = sorted(
        range(len(lengths)), key=lambda i: expressions[i] * lengths[i], reverse=True
    )
...
def get_transcript_stats(rnaspades_file, salmon_quantsf):
    ...
    transcript_ids = [
        extract_transcript_id(transcript.description) for transcript in transcripts
    ]
    ...
    if matched_lengths and matched_expressions:
        ...
        stats["Average Expression (TPM)"] = round(
            get_average_expression(matched_expressions), 2
        )
    else:
        stats["ExN50"] = "N/A"
...
def parse_arguments():
    ...
    parser.add_argument(
        "--eggnog", help="Path to eggNOG annotation file", required=True
    )
    parser.add_argument(
        "--salmon_metadata", help="Path to salmon quant metadata file", required=True
    )
    parser.add_argument("--fastp", help="Path to fastp JSON file", required=True)
    parser.add_argument(
        "--transcripts", help="Path to rnaspades assembly file", required=True
    )
