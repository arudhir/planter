...
class TestDuckDBCreation(unittest.TestCase):
    ...
    def _copy_fixture_files(self):
        ...
        shutil.copytree(
            self.fixtures_dir / self.sample_id, self.output_dir / self.sample_id
        )

        # Verify files were copied correctly
        transdecoder_file = (
            self.output_dir / self.sample_id / "transdecoder" / f"{self.sample_id}.pep"
        )
        eggnog_file = (
            self.output_dir
            / self.sample_id
            / "eggnog"
            / f"{self.sample_id}.emapper.annotations"
        )
        quant_file = (
            self.output_dir / self.sample_id / "quants" / f"{self.sample_id}.quant.json"
        )
    ...
    def test_duckdb_creation(self, mock_get_sra_info):
        ...
        logger.info(
            f"TransDecoder file path: {self.output_dir / self.sample_id / 'transdecoder' / f'{self.sample_id}.pep'}"
        )
        logger.info(
            f"EggNOG file path: {self.output_dir / self.sample_id / 'eggnog' / f'{self.sample_id}.emapper.annotations'}"
        )
        logger.info(
            f"Quant file path: {self.output_dir / self.sample_id / 'quants' / f'{self.sample_id}.quant.json'}"
        )
        ...
        with SequenceDBBuilder(
            str(self.test_db_path), output_dir=str(self.output_dir)
        ) as builder:
    ...
    def test_expression_table_loading(self):
        ...
        print(
            f"TPM Stats: Min={tpm_stats[0]}, Max={tpm_stats[1]}, Avg={tpm_stats[2]}, Median={tpm_stats[3]}"
        )
    ...
    def test_database_query_statistics(self):
        ...
        with SequenceDBBuilder(
            str(db_path), output_dir=str(self.output_dir)
        ) as builder:
            # Initialize the database schema
            builder.init_database()
            ...
            self.assertEqual(
                summary["sequences_with_expression"].values[0],
                3,
