...
class TestExpressionQueries(unittest.TestCase):
    ...
    def setUp(self):
        ...
        query_dir = (
            Path(__file__).parents[2] / "planter" / "database" / "queries" / "sql"
        )
    ...
    def test_expression_summary(self):
        ...
        result_sample = self.query_manager.sequences.get_expression_summary(
            self.sample_id
        )
    ...
    def test_expression_distribution(self):
        ...
        # Test with sample filter
        result_sample = self.query_manager.sequences.get_expression_distribution(
            self.sample_id
        )
    ...
    def test_top_expressed_sequences(self):
        ...
        # Test with sample filter
        result_sample = self.query_manager.sequences.get_top_expressed_sequences(
            limit=10, sample_id=self.sample_id
        )
    ...
    def test_sequence_expression(self):
        ...
        protein_seqhash = (
            "v1_DLS_bdfe3e5075584cd087ebd251e8ccdb41d07f712fef6076743611cc829c909ace.p1"
        )
        result = self.query_manager.sequences.get_expression_for_sequence(
            protein_seqhash
        )
    ...
    def test_annotation_to_expression(self):
        ...
        self.assertGreater(
            len(expression_data), 0, "Should find expression data for annotated gene"
        )
        ...
        self.assertGreater(
            len(joined_data), 0, "Should find joined data across all tables"
        )
        ...
        for row in joined_data:
            print(
                f"Protein: {row[0]} | Gene: {row[3]} | Name: {row[1]} | TPM: {row[4]}"
            )
        ...
        self.assertGreater(
            len(result), 0, "Should get results from annotation_with_expression"
        )
    ...
    def test_expression_level_categories(self):
        ...
        self.assertGreater(
            len(result), 1, "Should have multiple expression level categories"
        )
    ...
    def test_search_sequences(self):
        ...
        self.assertGreater(
            len(result),
            0,
            "Should find sequence with 'Phosphoglycerate' in description",
        )
        self.assertEqual(
            result.iloc[0]["description"], "Phosphoglycerate dehydrogenase"
        )
        ...
        self.assertGreater(
            len(result), 0, "Should find matching sequence with combined criteria"
        )
    ...
    def test_search_with_nonexistent_data(self):
        ...
        result = self.query_manager.sequences.search_sequences(
            description="NonexistentDescription12345"
        )
        self.assertEqual(
            len(result), 0, "Should not find any sequences with nonexistent description"
        )

        # Search with an impossible length range
        result = self.query_manager.sequences.search_sequences(
            min_length=10000, max_length=5000
        )
        self.assertEqual(
            len(result), 0, "Should not find any sequences with impossible length range"
        )

        # Search with nonexistent sample ID
        result = self.query_manager.sequences.search_sequences(
            sample_ids=["NonexistentSample12345"]
        )
        self.assertEqual(
            len(result), 0, "Should not find any sequences with nonexistent sample ID"
        )

        # Search with nonexistent GO term
        result = self.query_manager.sequences.search_sequences(go_terms=["GO:99999999"])
        self.assertEqual(
            len(result), 0, "Should not find any sequences with nonexistent GO term"
        )
